...
namespace Day23
{
    ...
    public class Network
    {
        ...
        private List<long> _nat = new();
        private List<long> _lastNat = new();
        private bool[] _idle = new bool[50];
        ...
        public void ActivateNat()
        {
            if (_idle.All(i => i) && _nat.Count == 2)
            {
                if (!_packages.TryGetValue(0, out var buffer))
                {
                    buffer = new();
                    _packages.Add(0, buffer);
                }

                buffer.Add(_nat[0]);
                buffer.Add(_nat[1]);

                if (_nat.Count == _lastNat.Count && _nat[1] == _lastNat[1])
                {
                ...
                _lastNat = _nat.ToList();
                _nat.Clear();
            }

            for (int i = 0; i < 50; i ++) _idle[i] = false;
        }
