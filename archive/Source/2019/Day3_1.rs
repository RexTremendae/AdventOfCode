...
fn walk(path: Vec<&str>, visited: &mut HashMap<(i32,i32), bool>) -> Option<i32>
{
    ...
    for p in path
    {
        let length = p[1..].parse::<i32>().unwrap();
        let movement = match p.chars().nth(0).unwrap()
        {
            'U' => (0,  1, length),
            'D' => (0, -1, length),
            'L' => (-1, 0, length),
            'R' => ( 1, 0, length),
            _ => panic!("Invalid movement specification")
        };
        let (mov_x, mov_y, length) = movement;
        ...
        for _ in 0..length
        {
            ...
            if visited.contains_key(&pos)
            {
                ...
                {
                    shortest = Some(cmp::min(shortest.unwrap(), len));
                }
            }
            else
            {
