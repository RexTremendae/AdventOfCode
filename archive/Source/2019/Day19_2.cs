...
namespace Day19
{
    public class Program
    {
        ...
        public void Run()
        {
            ...
            _instructions = File.ReadAllText("Day19.txt")
                                .Trim()
            ...
            int shipSize = 100;
            int xStart = 10, yStart = 7;

            _map = new Dictionary<(int, int), bool>();
            ...
            while (true)//xStart < 30)
            {
                bool found = true;
                int topRight = 0;
                int bottomLeft = 0;

                //WriteLine($"Testing {xStart}, {yStart}");

                for (var yLoop = 0; yLoop < shipSize; yLoop++)
                {
                    for (var xLoop = 0; xLoop < shipSize; xLoop++)
                    {
                        var x = xStart+xLoop;
                        var y = yStart+yLoop;
                        ...
                        if (IsPulled(x, y))
                        {
                            if (xLoop > yLoop) topRight++;
                            if (xLoop < yLoop) bottomLeft++;
                        }
                        else
                        {
                            found = false;
                ...
                if (found)
                {
                    WriteLine($"{xStart}, {yStart}");
                    WriteLine(xStart*10_000 + yStart);
                    return;
                }

                //WriteLine($"  {bottomLeft}\\{topRight}");
                if (topRight >= bottomLeft) xStart++;
                if (topRight <= bottomLeft) yStart++;
            }
        }

        private bool IsPulled(int x, int y)
        {
            if (!_map.TryGetValue((x, y), out var isPulled))
            {
                var drone = new TractorDrone(x, y);
                new Computer(new Memory(_instructions), drone).Run();
                isPulled = drone.IsPulled;
                _map[(x, y)] = isPulled;
            }
    ...
    public class TractorDrone : IConnector
    {
        ...
        public TractorDrone(int input1, int input2)
        {
            _inputs = new int[] { input1, input2 };
        }

        public long Output()
        {
            return _inputs[_inputIdx++];
        }

        public void ReceiveInput(long input)
        {
            IsPulled = input == 1 ? true : false;
        }
    }
