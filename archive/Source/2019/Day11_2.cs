...
namespace Day5
{
    public class Program
    {
        ...
        public void Run()
        {
            ...
            for (var y = maxY; y >= minY; y--)
            {
                ...
                {
                    if (!map.TryGetValue((x, y), out var pixel))
                    {
                        pixel = PaintColor.Black;
                    }

                    Write(pixel == PaintColor.White ? '█' : ' ');
                }
    ...
    public class Robot
    {
        public Dictionary<(long x, long y), PaintColor> PaintMap { get; set; }
        ...
        public Robot()
        {
            PaintMap = new Dictionary<(long x, long y), PaintColor>();
            _direction = Direction.Up;
            PaintMap[(0, 0)] = PaintColor.White;
        }
        ...
        public void ReceiveInput(long input)
        {
            if (_nextPaint == null)
            {
                ...
                PaintMap[_position] = _nextPaint.Value;
                return;
            }
        ...
        public long Output()
        {
            if (!PaintMap.TryGetValue(_position, out var paint))
            {
