...
namespace Day13
{
    public class Program
    {
        ...
        public void Run()
        {
            ...
            var instructions = File.ReadAllText("Day13.txt")
                                   ...
                                   .ToList();
            instructions[0] = 2;
            var arcade = new Arcade(interactive: false, printOutput: false);
    ...
    public class Arcade
    {
        ...
        private HorizontalMovement _horizontalDirection = HorizontalMovement.Unknown;
        private VerticalMovement _verticalDirection = VerticalMovement.Unknown;
        private (long x, long y) _ballPos;
        private (long x, long y) _paddlePos;

        private enum VerticalMovement { Unknown, Up, Down }
        private enum HorizontalMovement { Unknown, Left, Right }

        public Arcade(bool interactive, bool printOutput)
        {
            ...
            _ballPos = (long.MinValue, long.MinValue);
        }
        ...
        public void Print()
        {
            ...
            PrintStatus(blockCount);
        }

        public void PrintStatus(long? blockCount = null)
        {
            if (blockCount == null)
            {
                blockCount = _gameBoard.Count(x => x.Value == ArcadeEntity.Block);
            }
            WriteLine($"#blocks: {blockCount}, #score: {_score}");
        }

        private long Move()
        {
            if (_verticalDirection == VerticalMovement.Unknown ||
                _horizontalDirection == HorizontalMovement.Unknown) return 0;

            if (_horizontalDirection == HorizontalMovement.Left &&
                _ballPos.x < _paddlePos.x) return -1;

            if (_horizontalDirection == HorizontalMovement.Right &&
                _ballPos.x > _paddlePos.x) return 1;
        ...
        internal void ReceiveInput(long output)
        {
            ...
            if (i == 2)
            {
                ...
                if (x == -1 && y == 0)
                {
                    _score = _currentInput[2];
                    for (i = 0; i < 3; i++) _currentInput[i] = long.MinValue;
                    return;
                }

                var entity = (ArcadeEntity) _currentInput[2];
                ...
                if (entity == ArcadeEntity.Ball)
                {
                    if (_ballPos.x != long.MinValue)
                    {
                        _horizontalDirection = x - _ballPos.x > 0
                            ? HorizontalMovement.Right
                            : HorizontalMovement.Left;
                    }
                    if (_ballPos.y != long.MinValue)
                    {
                        _verticalDirection = y - _ballPos.y > 0
                ...
                else if (entity == ArcadeEntity.HorizontalPaddle)
                {
                    _paddlePos = (x, y);
                }
