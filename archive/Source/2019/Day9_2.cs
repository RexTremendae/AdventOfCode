...
namespace Day5
{
    public class Program
    {
        ...
        public void Run()
        {
            var instructions = File.ReadAllText("Day9.txt")
                                   .Trim()
                                   .Split(",", StringSplitOptions.RemoveEmptyEntries)
                                   .Select(x => long.Parse(x))
                                   .ToList();

            var computer = new Computer(new Memory(instructions), steadyInput: 2);
    ...
    public class Computer
    {
        ...
        {
            if (firstInput != null && steadyInput != null) throw new InvalidOperationException("Cannot provide both steady input and first input to computer.");
            _inputs = new List<long>();
            if (firstInput != null) _inputs.Add(firstInput.Value);
            if (steadyInput != null) _steadyInput = steadyInput;
            _memory = memory;
            _jumpTo = -1;
        ...
        {
            if (_steadyInput != null) return _steadyInput.Value;
            if (!_inputs.Any()) return null;
