...
namespace Day15
{
    public class Program
    {
        ...
        public void Run()
        {
            ...
            new OxygenFiller(repairDroid.Map).Fill(repairDroid.OxygenSystemPosition.Value);
        }
    }
    ...
    public class OxygenFiller
    {
        ...
        public OxygenFiller(Dictionary<(long x, long y), (Entity entity, long distance)> map)
        {
            _map = map;
        }

        private List<(long x, long y, long d)> _queue;

        public void Fill((long x, long y) oxygenSystemPosition)
        {
            _queue = new List<(long x, long y, long d)>();

            EnQ((oxygenSystemPosition.x, oxygenSystemPosition.y, 0));

            while (_queue.Any())
            {
                var q = _queue.First();
                _map[(q.x, q.y)] = (Entity.Oxygen, q.d);
                _max = Math.Max(_max, q.d);
                _queue.RemoveAt(0);
                EnQ(q);
            }
        ...
        private void EnQ((long x, long y, long d) item)
        {
            if (_map[(item.x+1, item.y)].entity == Entity.Empty)
            {
                _queue.Add((item.x+1, item.y, item.d+1));
            }
            if (_map[(item.x-1, item.y)].entity == Entity.Empty)
            {
                _queue.Add((item.x-1, item.y, item.d+1));
            }
            if (_map[(item.x, item.y+1)].entity == Entity.Empty)
            {
                _queue.Add((item.x, item.y+1, item.d+1));
            }
            if (_map[(item.x, item.y-1)].entity == Entity.Empty)
            {
                _queue.Add((item.x, item.y-1, item.d+1));
    ...
    public class RepairDroid : IConnector
    {
        ...
        public Dictionary<(long x, long y), (Entity entity, long distance)> Map => _map;
        public (long x, long y)? OxygenSystemPosition => _oxygenPosition;
        ...
        public long Output()
        {
            ...
            _totalMoves++;
            if (!RequestTermination && _totalMoves > 6 && _position == (0, 0))
            {
                WriteLine("Requested termination because droid is back at starting position");
                RequestTermination = true;
            }
            if (!RequestTermination && _totalMoves >= 5300)
            {
                WriteLine("Requested termination because of move limit");
                RequestTermination = true;
            }
        ...
        private Direction Move()
        {
            ...
            if (min.distance == long.MaxValue)
                throw new Exception($"No available moves! P: ({_position.x}, {_position.y})  T: {_totalMoves}");
