...
def enlarge(data):
  ...
  enlarged = []
  for line in data:
    enlarged_line = line.copy()
    for xmult in range(1,5):
      for x in range(width):
        n = enlarged_line[x] + xmult
        if n > 9: n -= 9
        enlarged_line.append(n)
    enlarged.append(enlarged_line)

  for ymult in range(1,5):
    for y in range(height):
      enlarged_line = enlarged[y].copy()
      for x in range(len(enlarged_line)):
        n = enlarged[y][x] + ymult
        if n > 9: n -= 9
        enlarged_line[x] = n
      enlarged.append(enlarged_line)
