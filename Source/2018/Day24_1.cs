...
namespace Day24
{
    public class Program
    {
        ...
        public IEnumerable<Group> ParseInput(string filename)
        {
            ...
            foreach (var line in File.ReadAllLines(filename).Select(x => x.Trim()))
            {
                ...
                var unitCount = int.Parse(groups["unitCount"].Value);
                var hitPoints = int.Parse(groups["hitPoints"].Value);
                var initiative = int.Parse(groups["initiative"].Value);
                var damageHitPoints = int.Parse(groups["damagePoints"].Value);
    ...
    public class Group
    {
        ...
        public int EffectivePower => UnitCount * DamageHitPoints;
    }
}
