...
namespace Day13
{
    ...
    public class Program
    {
        ...
        private void Run()
        {
            ...
            var allCarts = data.carts;
            while (allCarts.Count > 1)
            {
                var toRemove = new List<Cart>();
                var orderedCarts = allCarts.OrderBy(x => x.Position.Y).ThenBy(x => x.Position.X);
                foreach (var cart in orderedCarts)
                {
                    MoveCart(cart);
                    if (DetectCollision(cart, data.carts) is Cart otherCart)
                    {
                        toRemove.Add(cart);
                        toRemove.Add(otherCart);
                    }
                    TurnCart(cart, data.map[cart.Position.Y][cart.Position.X]);
                }

                foreach (var remove in toRemove) allCarts.Remove(remove);
            }

            var lastCart = allCarts.Single();
            WriteLine($"({lastCart.Position.X}, {lastCart.Position.Y})");
        }
        ...
        private Cart DetectCollision(Cart cart, IEnumerable<Cart> allCarts)
        {
